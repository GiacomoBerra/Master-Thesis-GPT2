...
class DataProcessing():
	...
	def load_DataLoaders(self):
		...
		train_dataset = torch.load(path.join('Data', 'train_dataset_doule.pt'))
		test_dataset = torch.load(path.join('Data', 'test_dataset_doule.pt'))
	...
	def createDataset(self):
					...
					for sent in history:
						...
						if len(sents) >= 1:
							for i, s in enumerate(sents):
								new_sent = [self.tokenizer.encode(self.tokens['bos_token'] + ' ' + s), self.tokenizer.encode(self.tokens['additional_special_tokens'][0]+ ' ' + flag + ' ' + self.tokens['eos_token'])]
								sents[i] = new_sent
								dataset_non_tokenized[flag] += [new_sent]

							n_sent[flag] += len(sents)

			with open(path.join('Data', 'dataset_non_tokenized_double.json'), 'w') as outfile:
				dump(dataset_non_tokenized, outfile)

			self.print_len_n_sent(review_len, n_sent)

		else:
			with open(path.join('Data', 'dataset_non_tokenized_double.json'), 'r') as json_file:
		...
		for name, dataset_list in dataset_tokenized.items():
			...
			for item in tqdm(dataset_list):
				...
				if item_len <= self.args.max_len + extra_length:
					...
					if [10378, 2790] == item[1][1:3]:
						item_1 = deepcopy(item[1])
						item_1[1:3] = [13159, 19095] # non depressed tokenized
						# labels to represent if the sentence is depressed or not
						dep_non_dep_lables.append([1])
					else:
						item_1 = deepcopy(item[1])
						item_1[1:3] = [10378, 2790] # depressed tokenized
						# labels to represent if the sentence is depressed or not
						dep_non_dep_lables.append([0])
					wrong = [item[0] + item_1 + self.new_tokens_encoded['pad_token'] * (self.args.max_len + extra_length - item_len), False]
		...
		torch.save(train_dataset, path.join('Data','train_dataset_doule.pt'))
		torch.save(test_dataset, path.join('Data','test_dataset_doule.pt'))
	...
	def train(self, engine, batch):
		...
		if engine.state.iteration % (floor(engine.state.epoch_length/10)+1) == 0 or engine.state.iteration == 1:
			...
			print("Evaluation")
			print('#'*100, end='\n\n')
			# initialize the tensors with the predictions and labels for the classification will be stored 
	...
	def start_training(self):
		...
		name = 'info_' + str(self.args.job_id) + '.json'
		with open(path.joinf(self.save_dir, name), 'w') as outfile:
			dump(self.train_info, outfile)
