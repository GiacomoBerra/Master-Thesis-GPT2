...
class DataProcessing():
	...
	def inference(self, engine, batch):
		...
		with torch.no_grad():
			batch = tuple(input_tensor.to(self.device) for input_tensor in batch)
			input_ids, lm_labels, token_type_ids, _ = batch
			...
			x = (lm_logits_flat_shifted.to(self.device), lm_labels_flat_shifted.to(self.device))
	...
	def compute_metrics(self, x):
		...
		self.eval_metrics['lm_loss'].append(float(loss(x[0], x[1])) / self.args.gradient_accumulation_steps)
	...
	def new_init_metrics(self):
		...
		self.eval_metrics = {'lm_loss':[], 'lm_perplexity':[]}
...
if __name__ == "__main__":
	...
	parser.add_argument("--max_norm", type=float, default=1, help="Max value that the norm of the gradient during the backpropagation can have.")
