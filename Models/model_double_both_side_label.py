...
#from sys import getsizeof, element_size, nelement
...
class DataProcessing():
	...
	def load_DataLoaders(self):
		...
		train_dataset = torch.load(path.join('Data', 'train_dataset_doule_both_lab.pt'))
		test_dataset = torch.load(path.join('Data', 'test_dataset_doule_both_lab.pt'))
	...
	def createDataset(self):
		...
		if False:
					...
					for sent in history:
						...
						if len(sents) >= 1:
							for i, s in enumerate(sents):
								new_sent = [self.tokenizer.encode(self.tokens['bos_token'] + ' ' + flag),
										    self.tokenizer.encode(self.tokens['additional_special_tokens'][0] + ' ' + s), 
										    self.tokenizer.encode(self.tokens['additional_special_tokens'][1]+ ' ' + flag + ' ' + self.tokens['eos_token'])]
			...
			with open(path.join('Data', 'dataset_non_tokenized_double_both_lab.json'), 'w') as outfile:
				dump(dataset_non_tokenized, outfile)

			self.print_len_n_sent(review_len, n_sent)

		else:
			with open(path.join('Data', 'dataset_non_tokenized_double_both_lab.json'), 'r') as json_file:
		...
		for name, dataset_list in dataset_tokenized.items():
			...
			for item in tqdm(dataset_list):
				item_len = len(item[0] + item[1] + item[2])
				...
				if item_len <= self.args.max_len + 2*extra_length:
					# Inputs
					correct = [item[0] + item[1] + item[2] + self.new_tokens_encoded['pad_token'] * (self.args.max_len + 2*extra_length - item_len), True]
					# if the current sentence is from a depressed subject 	
					if [10378, 2790] == item[0][1:3]:
						item_0, item_2 = deepcopy(item[0]), deepcopy(item[2])
						item_0[1:3], item_2[1:3] = [13159, 19095], [13159, 19095] # non depressed label tokenized
						# labels to represent if the sentence is depressed or not
						dep_non_dep_lables.append([1])
					else:
						item_0, item_2 = deepcopy(item[0]), deepcopy(item[2])
						item_0[1:3], item_2[1:3] = [10378, 2790], [10378, 2790] # depressed label tokenized
						# labels to represent if the sentence is depressed or not
						dep_non_dep_lables.append([0])
					wrong = [item_0 + item[1] + item_2 + self.new_tokens_encoded['pad_token'] * (self.args.max_len + 2*extra_length - item_len), False]
					# shuffle randomly the two sentences with the correct and wrong label 
					pre_input_id = [correct, wrong]
					# randomly shuffle the position of the correct and wrong sentence
					shuffle(pre_input_id)
					# append them to the input list
					input_ids.append([pre_input_id[0][0], pre_input_id[1][0]])

					# Index of the classification token in each input sequence
					if self.args.pos_class == 'end_lab':
						mc_token_ids.append([item_len - 1 for i in range(2)])
					elif self.args.pos_class == 'end_sent':
						mc_token_ids.append([len(item[0] + item[1]) for i in range(2)])

					# Labels for language modeling
					pre_lm_labels = deepcopy(pre_input_id)
					pre_lm_labels[0][0][item_len:] = [-100 for i in range(self.args.max_len + 2*extra_length - item_len)]
					pre_lm_labels[1][0][item_len:] = [-100 for i in range(self.args.max_len + 2*extra_length - item_len)]
					...
					local = [self.new_tokens_encoded['additional_special_tokens'][0]] * len(item[0]) + \
					        [self.new_tokens_encoded['additional_special_tokens'][1]] * len(item[1]) + \
					        [self.new_tokens_encoded['additional_special_tokens'][1]] * len(item[2]) + \
					         self.new_tokens_encoded['pad_token'] * (self.args.max_len + 2*extra_length - item_len)
		...
		torch.save(train_dataset, path.join('Data','train_dataset_doule_both_lab.pt'))
		torch.save(test_dataset, path.join('Data','test_dataset_doule_both_lab.pt'))
	...
	def train(self, engine, batch):
		...
		if engine.state.iteration % (floor(engine.state.epoch_length/10)+1) == 0 or engine.state.iteration == 1:
			...
			# evaluate the model
			self.evaluator.run(self.test_loader) #, epoch_length=2
	...
	def start_training(self):
		...
		print("\nStart the training\n")
		self.trainer.run(self.train_loader, max_epochs=self.args.n_epochs, epoch_length=ceil(len(self.train_loader)*self.args.perc_DataLoader_used_for_train)) # epoch_length=3
...
if __name__ == "__main__":
	...
	parser.add_argument("--perc_DataLoader_used_for_train", type=float, default=1, help="Percentage of the training data loader used for training.")
...
# TODO:
# change the keys of the dictionary at row 526, 527 and 507-512 and also in code plot_json.py
# capire metriche risultati huggingface per confronto

# Complited
# prendi le prime 5 parole di 500 frasi depresse e 500 non depresse e fai continuare dal modello 50 e 50 1000 con etichetta dep e non dep
# rimettere etichetta davanti e vedere se va
# run the test every n times during the training
# allenare 0.5 dep e distribuzione reale con DoubelHead
# splittare le loss de lm e mc
# compute also the F1, Recall and Precision a metrics
# Train SigleHeadLM model with 0 e 1 dep %
# complete the sentences using the trained model
# put label at the beginning and at the end 
# test if the label at the beginning has to be marked as part of the sentence in tensor "token_type_ids"

# python model_double_metrics_reInverted.py --job_id 0 --gradient_accumulation_steps 20 --depressed_perc 0.5 --create_DataLoaders False --perc_training 0.8 --n_epochs 1 --pos_class end_sent
