		...
		for name, dataset_list in dataset_tokenized.items():
			...
			for item in tqdm(dataset_list):
				...
				if item_len <= self.args.max_len + extra_length:
					...
					pre_lm_labels[0][0][item_len:] = [-100 for i in range(self.args.max_len + extra_length - item_len)]
					pre_lm_labels[1][0][item_len:] = [-100 for i in range(self.args.max_len + extra_length - item_len)]
